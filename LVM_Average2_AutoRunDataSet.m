...
for k = 1 : length(allFiles)    
    if(strfind(allFiles(k).name, '.lvm'))
        ...
        Vmean=mean(Ch4);
        Vmedian=median(Ch4); %this median is unused curently
        Vstd=std(Ch4);
        HVzeros=[HVzeros; 0];
        ...
        power=1.803*HVpp^2/(8*Rheater);
        valarr(count,:)=[HVpp power Vmean Vstd Vnorm Vstd Vnormroll Vstd];
...
saveas(gca,strcat(path, filex(1:length(filex)-4),'.png'), 'png');
%%
xcelfile=strcat(path,filex(1:sz-4),'means_ZeroNorms.xlsx');
