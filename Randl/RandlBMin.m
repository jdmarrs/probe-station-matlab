function [na,Sme,Hme,L] = RandlBMin(h1,T,l1,l2,lmin,ls,b1,b2,Ea,Vx,Tr)
%% Initial Variables
...
for t = 1:Tr % Tr is the number electron trials
   ...
   % Corner
        if (i == 2 && j == 1)
            G1R = PVB(T,L(i,j + 1,1),Ea,Vx,L(i,j + 1,2));
            G1L = PVB(T,L(i - 1,j + 1,1),Ea,Vx,L(i - 1,j + 1,2));
            G2R = NOVB(T,L(i + 1,j,1),Ea,Vx,L(i + 1,j,2));
            [i1,j1] = Ad3C1PR(i,j,g,G1R,G1L,G2R);
        elseif (j == 1 && i == xm2)
            G1R = PVB(T,L(i,j + 1,1),Ea,Vx,L(i,j + 1,2));
            G1L = PVB(T,L(i - 1,j + 1,1),Ea,Vx,L(i - 1,j + 1,2));
            G2L = NOVB(T,L(i - 1,j,1),Ea,Vx,L(i - 1,j,2));
    ...
    % on the sides of the longer rows
        elseif i == 1
            G1R = PVB(T,L(i,j + 1,1),Ea,Vx,L(i,j + 1,2));
            G2R = NOVB(T,L(i + 1,j,1),Ea,Vx,L(i + 1,j,2));
            G3R = NVB(T,L(i,j - 1,1),Ea,Vx,L(i,j - 1,2));
            [i1,j1] = Ad3SCNPR(i,j,g,G1R,G2R,G3R);
        elseif i == xm1
             G1L = PVB(T,L(i - 1,j + 1,1),Ea,Vx,L(i - 1,j + 1,2));
             G2L = NOVB(T,L(i - 1,j,1),Ea,Vx,L(i - 1,j,2));
             G3L = NVB(T,L(i - 1,j - 1,1),Ea,Vx,L(i - 1,j - 1,2));
