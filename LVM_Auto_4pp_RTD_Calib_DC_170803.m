...
for k = 1 : length(allFiles)    
    if(strfind(allFiles(k).name, '.lvm'))
        ...
        valarr(count,:)=[HVpp DT Rdut gof.rsquare Hcurr];
        
        if gof.rsquare>0.82
           
           gooddata(count2,:)= [HVpp DT Rdut gof.rsquare Hcurr];
           count2=count2+1;
...
subplot(2,2,2)
errorbar(valarr(:,1)/2, valarr(:,3), valarr(:,4), 'bs');
title('No Zero Normalization');
xlabel('HV_D_C'); ylabel('R');
...
subplot(2,2,4)
errorbar(gooddata(:,1)/2, gooddata(:,3), gooddata(:,4), 'bs');
title('No Zero Normalization');
xlabel('HV_D_C'); ylabel('R');
...
%}
saveas(gca,strcat(path, filex(1:length(filex)-4),date,'_wHcurr.png'), 'png');
saveas(gca,strcat(path, filex(1:length(filex)-4),date,'_wHcurr.fig'), 'fig');
%% temperature
...
%%
xcelfile=strcat(path,filex(1:sz-4),date,'_Means_ZeroNorms_wHCurr.xlsx');
headers={'MeasNum','HVpp', 'DT', 'R (Ohms)','GoF', 'Hcurr'};
