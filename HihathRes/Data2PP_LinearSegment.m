...
amp=1E-8; %set amplification for V4 in Amps/Volt, Inverting

valarr=zeros(numfiles, 5, 'double');
filex=filename;
measnum=filex(sz-5:sz-4);
colors=['y','m','c','r','g','b','k','y','m','c','r','g','b','k','y','m','c','r','g','b','k','y','m','c','r','g','b','k'];

for i=1:numfiles    
    %data portion
    dat=importdata(strcat(path,filex),',',3);
    newdat=dat.data(890:1100,:); %requires that OScope is in Normal Trigger
    V1=newdat(1:size(newdat,1),2); % v1
    I4=amp.*newdat(1:size(newdat,1),3); 
...
xcelfile=strcat(path,filex(1:sz-4),'_seg.xlsx');
