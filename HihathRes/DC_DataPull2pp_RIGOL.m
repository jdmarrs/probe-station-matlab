...
%amp=10^-2; %set amplification for V4 in Amps/Volt, Inverting

valarr=zeros(1, 7, 'double');
filex=filename;
...
for k = 1 : length(allFiles)
    ...
    if(strfind(allFiles(k).name, '.csv'))
        ...
        measnum=filex(sz-5:sz-4);
        valarr(count, :)=[mean(V1) mean(V2) mean(V12) median(V1) median(V2) mode(V1) mode(V2)];
...
headers={'Filename','mean(V1)','mean(V2)', 'mean(V1-V2)','median(V1)', 'median(V2)', 'mode(V1)', 'mode(V2)'};
