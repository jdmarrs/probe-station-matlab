...
resin=input('What is image resolution?  answer 1024 or 2048 only ');
...
scale=3.420E-6*scalein; %pix/nm for 1,000,000 x magnification
pix2nm=1/scale; %nm/pix
largestparticle=input('max: input largest particle radius in nm: '); %nm radius
minparticle=input('min: input smallers particle radius in nm: '); %nm radius
...
while strcmp(repeat,'y');
    ...
    repeat=input('want to repeat this process? y or n ', 's');
