...
for k = 1 : 1
    ...
    if 1 %(strfind(allFiles(k).name, '.tif'))

        %filename=allFiles(k).name;
        OImg=strcat(path, filename1);
        imTif = imread(OImg);
        
        imTif2=imcrop(imTif, [1 1 1024 882]);  % for normal Res
        ...
        scale=1.715E-6*scalein; %pix/nm for 1,000,000 x magnification
